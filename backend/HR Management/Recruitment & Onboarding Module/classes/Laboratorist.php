...
class Laboratorist {
    ...
    public function getLaboratorists($search = '', $status = '') {
        $sql = "SELECT * FROM hr_employees WHERE profession = 'Laboratorist'";
        $conditions = [];
        $params = [];
        $types = '';
    ...
    public function addLaboratorist($data) {
        if (isset($data['specialization']) && $data['specialization'] === "Others" && !empty($data['other_specialization'])) {
            $data['specialization'] = $data['other_specialization'];
        }
    ...
    public function getByEmployeeId($employeeId) {
        $stmt = $this->conn->prepare("SELECT * FROM hr_employees WHERE employee_id = ? AND profession = 'Laboratorist' LIMIT 1");
