...
class Doctor {
    ...
    public function getDoctors($search = '', $status = '') {
        $sql = "SELECT * FROM hr_employees WHERE profession = 'Doctor'";
        $conditions = [];
        $params = [];
        $types = '';
    ...
    public function getByEmployeeId($employeeId) {
        $stmt = $this->conn->prepare("SELECT * FROM hr_employees WHERE employee_id = ? AND profession = 'Doctor' LIMIT 1");
