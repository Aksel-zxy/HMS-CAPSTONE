...
$accountant = new Accountant($conn);
$uploader = new FileUploader($conn);

$search = $_GET['search'] ?? '';
$status = $_GET['status'] ?? '';
$accountants = $accountant->getAccountants($search, $status);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = $_POST;
    $data['password'] = password_hash($data['password'], PASSWORD_DEFAULT);

    if ($accountant->employeeExists($data['employee_id'])) {
        echo "<script>alert('Error: Employee ID already exists!'); history.back();</script>";
        exit;
    }

    if ($accountant->addAccountant($data)) {
        $uploader->uploadDocuments($data['employee_id'], $_FILES);
        echo "<script>alert('Accountant registered successfully!'); window.location='list_of_accountants.php';</script>";
    } else {
        echo "<script>alert('Error registering Accountant!');</script>";
    }
}
...
<body>
    ...
    <div class="main">
        ...
        <div class="topbars">
            ...
            <div class="search-bar">
                <form method="GET" action="list_of_accountants.php">
        ...
        <!-- ----- Add Accountant ----- -->
        <center>
            <button class="hahaha" onclick="openForm()">Add Accountant</button>
        </center>

        <!-- ----- Table ----- -->
        <div class="employees">
            <p style="text-align: center;font-size: 35px;font-weight: bold;padding-bottom: 20px;color: black;">Accountants List</p>
            <table id="AccountantsTable">
                ...
                <tbody>
                    <?php if ($accountants && $accountants->num_rows > 0): 
                        $i = 1;
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <center>
                                    <a href="view_accountant.php?employee_id=<?= htmlspecialchars($row['employee_id']); ?>" class="view-link">View Details</a>
                                </center>
                            </td>
                        </tr>
                    <?php endwhile; 
                    ...
                    ?>
                        <tr>
                            <td colspan="8" style="text-align:center;">No Accountant found</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <!-- ----- Pop-Up Form (Add Accountant) ----- -->
        <div id="popupForm" class="popup-form">
            <div class="form-container">
                <bttn class="close-btn" onclick="closeForm()">X</bttn>
                <center>
                    <h3 style="font-weight: bold;">Add Accountant</h3> 
                </center>
                ...
                <select id="degree_type" name="degree_type" required>
                    ...
                    <option value="Bachelor of Science in Management Accounting (BSMA)">Bachelor of Science in Management Accounting (BSMA)</option>
                    <option value="Bachelor of Science in Internal Auditing">Bachelor of Science in Internal Auditing</option>
                    <option value="Bachelor of Science in Business Administration Major in Financial Management (BSBA-FM)">Bachelor of Science in Business Administration Major in Financial Management (BSBA-FM)</option>
                </select>
                <br />
    ...
    <script>
        ...
        function updateUsername() {
            ...
            if (/^\n{3}$/.test(employeeId)) {
                usernameField.value = "a" + employeeId;
            } else {
                usernameField.value = "";
            }
        }
