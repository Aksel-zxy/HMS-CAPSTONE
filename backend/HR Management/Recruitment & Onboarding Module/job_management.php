...
if (!$user) {
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO hr_job (title, job_position, job_description, specialization, image) VALUES ('$title', '$job_position', '$job_description', '$specialization', '$image_path')";

            if ($conn->query($sql)) {
                echo "<script>alert('Job Post added successfully!'); window.location.href = 'job_management.php';</script>";
            } else {
                echo "Failed to save job post: " . $con->error;
            }
        } else {
  ...
  if (isset($_GET['id'])) {
    ...
    $sql = "SELECT image, title, job_position, job_description, specialization, FROM hr_job WHERE job_id = ?";
    $stmt = $con->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->store_result();
    $stmt->bind_result($image, $title);

    if ($stmt->num_rows > 0) {
        $stmt->fetch();

        $sql = "DELETE FROM hr_job WHERE id = ?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param("i", $id);
        $stmt->execute();

        header('Location: job_management.php?message=Job Post deleted successfully.');
        exit;
    } else {
...
<head>
    ...
    <link rel="stylesheet" href="css/job_management.css">
</head>
...
<body>
    <div class="d-flex">
        ...
        <div class="main">
            ...
            <!-- ----- Job Post Upload ----- -->
            <div class="job_post">
                <h2>Post a Job Vaccance</h2>
                <div class="job_post-box">
                    ...
                    <?php
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                echo '<div class="job_post-item" style="border: 1px solid #ccc; padding: 15px; margin-bottom: 10px; border-radius: 10px;">';
                                    ...
                                    echo '<div style="text-align:center; margin-top:10px;">';
                                        echo '<a href="job_management.php?job_id=' . $row['job_id'] . '" onclick="return confirm(\'Are you sure you want to delete this job post?\');" style="color:red; text-decoration:none;">Delete</a>';
                                    echo '</div>';
