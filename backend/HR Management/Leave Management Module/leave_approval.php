...
require 'classes/LeaveApproval.php';
...
$leaveApproval = new LeaveApproval($conn);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['leave_id'])) {
    ...
    if ($leaveApproval->approveOrRejectLeave($leave_id, $action)) {
        header("Location: leave_approval.php?msg=success");
        exit;
    } else {
        echo "<div class='alert alert-danger'>Failed to update leave request.</div>";
    }
}
...
$leaveBalances = [];
$result = $leaveCreditModel->getAllLeaveCredits($currentYear);
while ($row = $result->fetch_assoc()) {
    $leaveBalances[$row['employee_id']] = $row['remaining_leave_days'];
}

$pendingLeaves = $leaveApproval->fetchPendingLeaves();
        ...
        <div class="main">
            ...
            <?php
                if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action'], $_POST['leave_id'])) {
                    ...
                    $stmt = $conn->prepare("UPDATE hr_leave SET leave_status = ?, approval_date = ? WHERE leave_id = ?");
                    $stmt->bind_param("ssi", $status, $approval_date, $leave_id);
                    $stmt->execute();
                }
            ...
            <div class="employees">
                <p style="text-align: center; font-size: 35px; font-weight: bold; padding-bottom: 20px; color: black;">Leave Applications for Approval</p>
                    ...
                    <tbody>
                        <?php if ($pendingLeaves && $pendingLeaves->num_rows > 0): ?>
                            <?php while ($row = $pendingLeaves->fetch_assoc()):
                                ...
                                $remaining = isset($leaveBalances[$row['employee_id']]) ? (int)$leaveBalances[$row['employee_id']] : 0;
                                ...
                                <td>
                                    <?php if ($insufficient): ?>
                                        <span style="color:red;font-weight:bold;">
                                            Insufficient Balance (<?= $remaining ?> left, needs <?= $daysRequested ?>)
                                        </span>
                                    <?php else: ?>
                                        <?= $remaining ?> days
                                    <?php endif; ?>
