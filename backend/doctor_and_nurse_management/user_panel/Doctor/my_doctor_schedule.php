...
// 2. NEW: Fetch Rooms Lookup List
// We fetch all rooms so we can swap IDs (e.g., 101) for Names (e.g., 'Room 303') later
$rooms_lookup = [];
...
if ($room_result) {
    while ($r_row = $room_result->fetch_assoc()) {
        // Creates an array like: [1 => 'ICU', 2 => 'OPD Room 1']
        $rooms_lookup[$r_row['room_id']] = $r_row['room_name'];
    }
}
// 3. NEW: Fetch Nurse Schedules and Map them
// We organize data like: $nurse_lookup['2024-01-20']['mon'][101] = "Nurse Jane";
$nurse_lookup = [];

// Join shift_scheduling with hr_employees to get Nurse names
...
if ($nurse_result) {
    while ($n_row = $nurse_result->fetch_assoc()) {
        ...
        // Loop through days to store the nurse's assigned room for each day
        $day_prefixes = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'];

        foreach ($day_prefixes as $d_prefix) {
            $r_id = $n_row[$d_prefix . '_room_id'];

            // If the nurse has a room assigned this day
            if (!empty($r_id)) {
                // We use an array because multiple nurses might be in one room
                $nurse_lookup[$week][$d_prefix][$r_id][] = $nurse_name;
            }
        }
    }
}
// 3. Fetch schedule for the logged-in doctor
$modal_schedules = [];
...
<body>
        ...
        <aside id="sidebar" class="sidebar-toggle">
            ...
            <div class="text-center my-4">
                <a href="#" class="d-inline-block text-decoration-none profile-trigger" data-bs-toggle="modal" data-bs-target="#profileModal">
                    ...
                    <div class="mt-2 text-primary fw-bold" style="font-size: 14px;">View Profile</div>
                </a>
            </div>
    ...
    </div> <?php include 'doctor_profile.php'; ?>
    <script>
