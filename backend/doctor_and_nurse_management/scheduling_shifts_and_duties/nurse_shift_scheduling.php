...
foreach ($nurses as $nurse) {
    $role_map[$nurse['employee_id']] = $nurse['role'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_schedule'])) {
    ...
    if ($stmt->execute()) {
        header("Location: nurse_shift_scheduling.php?view_sched_id=" . urlencode($employee_id) . "&success=update");
        exit();
    }
}
    ...
    <div class="d-flex">
                    ...
                    <div class="card-body bg-white rounded">
                        ...
                        <form method="POST" class="mb-4">
                            <div class="row g-3 mb-4">
                                <div class="col-md-4">
                                    <label for="employee_id">Select Nurse</label>
                                    <select name="employee_id" id="nurse_select" class="form-select" required>
                                        <option value="">-- Choose Nurse --</option>
                                        <?php foreach ($nurses as $nurse): ?>
                                            <option value="<?= $nurse['employee_id'] ?>"><?= $nurse['first_name'] ?> <?= $nurse['last_name'] ?> (<?= $nurse['role'] ?>)</option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="col-md-3">
                    ...
                    <div class="card-body bg-white rounded">
                        <table class="table table-bordered table-striped nurses-list-table rounded shadow-sm">
                            ...
                            <tbody>
                                <?php foreach ($nurses as $nurse): ?>
                                    <tr>
                                        <td><?= htmlspecialchars($nurse['employee_id']) ?></td>
                                        <td><?= htmlspecialchars($nurse['first_name']) ?></td>
                                        <td><?= htmlspecialchars($nurse['middle_name']) ?></td>
                                        <td><?= htmlspecialchars($nurse['last_name']) ?></td>
                                        <td><?= htmlspecialchars($nurse['role']) ?></td>
                                        <td><?= htmlspecialchars($nurse['profession']) ?></td>
                                        <td><?= htmlspecialchars($nurse['department']) ?></td>
                                        <td>
                                            <form method="get" style="display:inline;">
                                                <input type="hidden" name="view_sched_id" value="<?= htmlspecialchars($nurse['employee_id']) ?>">
                                        ...
                                        <td>
                                            <form action="nurse_download_schedule.php" method="get" target="_blank">
                                                <input type="hidden" name="employee_id" value="<?= htmlspecialchars($nurse['employee_id']) ?>">
                            ...
                            <div class="modal-content shadow-lg border-0" style="max-height: 90vh;">
                                <div class="modal-header bg-primary text-white">
                                    <h5 class="modal-title">
                                        ...
                                        Schedules for Nurse ID: <?= htmlspecialchars($view_id) ?>
                                    </h5>
                                    <a href="nurse_shift_scheduling.php" class="btn-close btn-close-white"></a>
                                </div>
                                ...
                                <div class="modal-body p-4" style="overflow-y: auto; background-color: #f8f9fa;">
                                    ...
                                    <?php else: ?>
                                        <div class="text-center py-5">
                                            ...
                                            <p>This nurse currently has no shifts assigned to them.</p>
                                        </div>
                                    <?php endif; ?>
                                </div>
                                <div class="modal-footer bg-light">
                                    <a href="nurse_shift_scheduling.php" class="btn btn-primary px-4">Close View</a>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                const nurseSelect = document.getElementById('nurse_select');
                    ...
                    if (empId && weekDate) {
                        ...
                        fetch('check_sched.php', {
                            ...
                            .then(data => {
                                ...
                                if (result === 'exists') {
                                    statusDiv.innerHTML = '<span class="text-danger">❌ Week already scheduled for this nurse!</span>';
                                    saveBtn.disabled = true; // STOPS YOU FROM SUBMITTING

                                    Swal.fire({
                                        icon: 'error',
                                        title: 'Duplicate Detected',
                                        text: 'This nurse already has a schedule for the chosen week.',
                ...
                nurseSelect.addEventListener('change', checkAvailability);
        ...
        <script type="text/javascript">
            ...
            if (errorMsg) {
                Swal.fire({
                    ...
                    window.location.href = 'nurse_shift_scheduling.php';
                });
            }
