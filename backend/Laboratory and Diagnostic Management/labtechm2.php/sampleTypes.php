...
require_once 'sampleTypes_class.php';
...
//main content
$labdiagnostic_sampletypes = new labdiagnostic_sampletypes($conn);

//get records/dropdown
$sampleTypes = $conn->query("SELECT sample_type_id FROM labdiagnostic_sampletypes
ORDER BY sample_type_id ASC")->fetch_all(MYSQLI_ASSOC);
$sampleTypes = $conn->query("SELECT * FROM labdiagnostic_sampletypes");

// Handle form submissions
$message = '';
$error = '';
$sampleTypeToEdit = null;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['create'])) {
        $labdiagnostic_sampletypes->name = $_POST['name'];
        $labdiagnostic_sampletypes->code = $_POST['code'];
        $labdiagnostic_sampletypes->storage_requirements = $_POST['storage_requirements'];
        $labdiagnostic_sampletypes->stability_duration = $_POST['stability_duration'];

        if ($labdiagnostic_sampletypes->create()) {
            $_SESSION['message'] = "Sample type added successfully.";
        } else {
            $_SESSION['error'] = "Unable to add sample type: " . $conn->error;
        }
        header('Location: sampleTypes.php');
        exit();
    }
    elseif (isset($_POST['update'])) {
        $labdiagnostic_sampletypes->sample_type_id = $_POST['sample_type_id'];
        $labdiagnostic_sampletypes->name = $_POST['name'];
        $labdiagnostic_sampletypes->code = $_POST['code'];
        $labdiagnostic_sampletypes->storage_requirements = $_POST['storage_requirements'];
        $labdiagnostic_sampletypes->stability_duration = $_POST['stability_duration'];

        if ($labdiagnostic_sampletypes->update()) {
            $_SESSION['message'] = "Sample type updated successfully.";
        } else {
            $_SESSION['error'] = "Unable to update sample type: " . $conn->error;
        }
        header('Location: sampleTypes.php');
        exit();
    }
    elseif (isset($_POST['delete'])) {
        ...
        if ($labdiagnostic_sampletypes->delete($sample_type_id)) {
            $_SESSION['message'] = "Sample type deleted successfully.";
        } else {
            $_SESSION['error'] = "Unable to delete sample type: " . $conn->error;
        }
        header('Location: sampleTypes.php');
        exit();
    }
...
// Handle edit request
if (isset($_GET['edit'])) {
    $sample_type_id = $_GET['edit'];
    $sampleTypeToEdit = $labdiagnostic_sampletypes->getById($sample_type_id);
}

// Get all sample types
$sampleTypes = $labdiagnostic_sampletypes->getAll();
...
<!----- End of Sidebar ----->
    ...
    <h2><?php echo $sampleTypeToEdit ? 'Edit Sample Type' : 'Add New Sample Type'; ?></h2>
    <form id="sampleTypeForm" method="post" action="sampleTypes.php">
        <input type="hidden" name="sample_type_id" value="<?php echo $sampleTypeToEdit ? $sampleTypeToEdit['sample_type_id'] : ''; ?>">

        <div class="form-group">
            <label for="name">Name:</label>
            <input type="text" name="name" required value="<?php echo htmlspecialchars($sampleTypeToEdit['name'] ?? ''); ?>">
        </div>

        <div class="form-group">
            <label for="code">Code:</label>
            <input type="text" name="code" required value="<?php echo htmlspecialchars($sampleTypeToEdit['code'] ?? ''); ?>">
        </div>

        <div class="form-group">
            <label for="storage_requirements">Storage Requirements:</label>
            <textarea name="storage_requirements" rows="3"><?php echo htmlspecialchars($sampleTypeToEdit['storage_requirements'] ?? ''); ?></textarea>
        </div>

        <div class="form-group">
            <label for="stability_duration">Stability Duration:</label>
            <input type="text" name="stability_duration" required value="<?php echo htmlspecialchars($sampleTypeToEdit['stability_duration'] ?? ''); ?>">
        </div>

        <div class="form-buttons">
            <?php if ($sampleTypeToEdit): ?>
                <button type="submit" name="update" class="btn btn-warning">Update</button>
                <a href="sampleTypes.php" class="btn btn-secondary">Cancel</a>
            <?php else: ?>
        ...
        <tbody>
            <?php if (!empty($sampleTypes)): ?>
                <?php foreach ($sampleTypes as $sampleType): ?>
                    <tr data-sample-type-id="<?php echo htmlspecialchars($sampleType['sample_type_id']); ?>"
                        data-name="<?php echo htmlspecialchars($sampleType['name']); ?>"
                        data-code="<?php echo htmlspecialchars($sampleType['code']); ?>"
                        data-storage-requirements="<?php echo htmlspecialchars($sampleType['storage_requirements']); ?>"
                        data-stability-duration="<?php echo htmlspecialchars($sampleType['stability_duration']); ?>">

                        <td><?php echo htmlspecialchars($sampleType['sample_type_id']); ?></td>
                        <td><?php echo htmlspecialchars($sampleType['name']); ?></td>
                        <td><?php echo htmlspecialchars($sampleType['code']); ?></td>
                        <td><?php echo htmlspecialchars($sampleType['storage_requirements']); ?></td>
                        <td><?php echo htmlspecialchars($sampleType['stability_duration']); ?></td>
                        <td>
                            <button class="btn-view" data-id="<?php echo $sampleType['sample_type_id']; ?>">View</button>
                            <button class="btn-edit" data-id="<?php echo $sampleType['sample_type_id']; ?>">Edit</button>
                            <form method="POST" style="display: inline;">
                                <input type="hidden" name="sample_type_id" value="<?php echo $sampleType['sample_type_id']; ?>">
                                <button type="submit" name="delete" class="btn-delete" onclick="return confirm('Are you sure you want to delete this Sample type?')">Delete</button>
                            </form>
                    </td>
                    </tr>
                <?php endforeach; ?>
                <?php else: ?>
                <tr>
                    <td colspan="9">No sample types available.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
...
<!--viewing modal-->
        <div id="sampleTypesModal" class="modal">
            <div class="modal-content">
                <span class="close">&times;</span>
                <h2>Sample Types Details</h2>
                <div id="sampleTypesDetails">
                    <!--loaded here-->
                </div>
            </div>
        </div>
    </div>
    <script src="../assets/Bootstrap/sampleTypes.js"></script>  
