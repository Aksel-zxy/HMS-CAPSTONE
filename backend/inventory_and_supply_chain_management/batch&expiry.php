...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .alert-close { float:right; background:none; border:none; font-size:1.1rem; cursor:pointer; color:#8a6300; padding:0; line-height:1; }
        ...
        .modal-box { background:var(--card);border-radius:var(--radius);box-shadow:var(--shadow-lg);width:100%;max-width:480px;padding:0;position:relative;margin:16px;overflow:hidden;animation:slideUp .25s ease; }
        ...
        /* Category-Type pair display */
        .cat-type-preview {
            display:flex;align-items:center;gap:8px;
            background:var(--bg);border:1px solid var(--border);
            border-radius:8px;padding:10px 14px;margin-bottom:16px;
        }
        .cat-type-preview .preview-label { font-size:.72rem;font-weight:800;color:var(--text);text-transform:uppercase;letter-spacing:.04em; }
        .cat-type-preview .preview-val { font-size:.85rem;font-weight:700;color:var(--text-dark);margin-top:2px; }
        .preview-sep { color:var(--border);font-size:1.2rem;margin:0 4px; }
                                ...
                                <td style="min-width:200px;">
                                    <div style="display:flex;flex-direction:column;gap:4px;">
                                        <?php if (!empty($nd['category'])): ?>
                                            <span class="badge badge-type" style="font-size:.7rem;"><?= htmlspecialchars($nd['category']) ?></span>
                                        <?php else: ?>
                                            <span class="badge badge-muted" style="font-size:.7rem;">No category</span>
                                        <?php endif; ?>
                                        <?php if (!empty($nd['item_type'])): ?>
                                            <span class="badge badge-purple" style="font-size:.7rem;"><?= htmlspecialchars($nd['item_type']) ?></span>
                                        <?php else: ?>
                                            <span class="badge badge-muted" style="font-size:.7rem;">No type</span>
                                        <?php endif; ?>
                                        <button type="button" class="btn btn-sm"
                                            style="padding:3px 10px;font-size:.75rem;background:var(--primary-light);color:var(--primary);border:1px solid rgba(0,172,193,.3);margin-top:2px;width:fit-content;"
...
<div class="modal-overlay" id="catModal">
    <div class="modal-box" style="max-width:500px;">
        <div class="modal-head" style="background:var(--primary);">
            ...
            <p id="catModalSubtitle" style="font-size:.82rem;opacity:.85;margin-top:3px;">Choose a category, then pick the specific item type</p>
        </div>
        <form method="post">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label>
                        <span style="display:inline-flex;align-items:center;gap:6px;">
                            <span style="background:var(--primary);color:#fff;font-size:.7rem;font-weight:800;padding:2px 7px;border-radius:20px;">1</span>
                            Category <span style="color:var(--danger);">*</span>
                        </span>
                    </label>
                    <select name="new_category" id="catCategory" class="form-control" required onchange="onCategoryChange()">
                        <option value="">— Select Category —</option>
                        <option value="Medicine & Pharmacy">💊 Medicine & Pharmacy</option>
                        <option value="Diagnostic & Equipment">🔬 Diagnostic & Equipment</option>
                        <option value="Therapeutic & Surgical">🩺 Therapeutic & Surgical</option>
                ...
                <!-- Step 2: Item Type (populated based on category) -->
                <div class="form-group" id="itemTypeGroup" style="display:none;">
                    <label>
                        <span style="display:inline-flex;align-items:center;gap:6px;">
                            <span style="background:var(--purple);color:#fff;font-size:.7rem;font-weight:800;padding:2px 7px;border-radius:20px;">2</span>
                            Item Type <span style="color:var(--danger);">*</span>
                        </span>
                    </label>
                    <select name="new_item_type" id="catItemType" class="form-control" required>
...
<script>
    ...
    const categoryTypeMap = {
        ...
        'Laboratory Supplies': [
            ...
            'Test Kits and Rapid Diagnostics',
            'Culture Media and Specimens',
            'Microscopy Supplies'
        ],
        'IT & Technology': [
    ...
    function openCatModal(itemName, currentCat, currentType) {
        ...
        // Populate item types for the pre-selected category
        onCategoryChange();

        // Pre-select current item type if it exists in the populated list
        if (currentType) {
            for (let opt of typeSel.options) {
                if (opt.value.toLowerCase() === currentType.toLowerCase()) {
    ...
    function closeCatModal() {
        ...
        document.getElementById('catItemType').innerHTML       = '<option value="">— Select Item Type —</option>';
        document.getElementById('itemTypeGroup').style.display = 'none';
    }
