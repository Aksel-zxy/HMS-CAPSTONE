...
/* =====================================================
   FETCH BILLING RECORD — keyed on billing_id
===================================================== */
$stmt = $conn->prepare("
    ...
    try {
        ...
        if ($linkStatus === 'paid') {
            $s = $conn->prepare("
                ...
                SET status='Paid', payment_method='PAYMONGO', payment_date=NOW(), paymongo_link_id=?
                WHERE billing_id=?
            ");
            ...
            $s2 = $conn->prepare("
                ...
                ON DUPLICATE KEY UPDATE
                    ...
                    paymongo_reference = VALUES(paymongo_reference)
            ");
            $s2->bind_param("iiss", $patient_id, $billing_id, $existing_link, $existing_link);
            $s2->execute();
...
/* =====================================================
   FETCH PATIENT RECEIPT — keyed on billing_id
===================================================== */
...
if ($grand_total <= 0 && $status !== 'Paid') {
    ...
    $s2 = $conn->prepare("
        ...
        VALUES (?, ?, 'Paid', 'Insurance', ?)
        ON DUPLICATE KEY UPDATE
            status            = 'Paid',
            payment_method    = 'Insurance',
            payment_reference = VALUES(payment_reference)
    ");
    $s2->bind_param("iis", $patient_id, $billing_id, $ins_ref);
    $s2->execute();
...
if (isset($_POST['payment_method']) && $_POST['payment_method'] === 'cash' && $grand_total > 0) {
    ...
    $s = $conn->prepare("
        INSERT INTO paymongo_payments
            (payment_id, amount, status, payment_method, remarks, billing_id, patient_id, paid_at)
    ...
    $s2 = $conn->prepare("
        ...
        SET status='Paid', payment_method='Cash', payment_date=NOW(),
            paymongo_payment_id=?, paid_amount=?
        WHERE billing_id = ?
    ");
...
if ($grand_total > 0 && $status !== 'Paid') {
    if ($existing_link) {
        try {
            $linkResp   = $paymongoClient->get("links/{$existing_link}");
            $linkData   = json_decode($linkResp->getBody(), true);
            $payLinkUrl = $linkData['data']['attributes']['checkout_url']
                          ?? "https://checkout.paymongo.com/links/{$existing_link}";
        } catch (Exception $e) {
...
/* =====================================================
   PAYMENT HISTORY — keyed on billing_id
===================================================== */
$s = $conn->prepare("
...
$s->bind_param("is", $billing_id, $like);
$s->execute();
$history = $s->get_result()->fetch_all(MYSQLI_ASSOC);

/* Receipt ID for "View Receipt" link */
...
body {
    ...
    min-height: 100vh; color: var(--gray-800); font-size: 14px; line-height: 1.6;
}
.cw { margin-left: 250px; padding: 40px 32px 80px; transition: margin-left .3s ease; }
.cw.sidebar-collapsed { margin-left: 0; }
.billing-center { max-width: 760px; margin: 0 auto; }

/* TOP NAV */
.top-nav { display:flex;align-items:center;justify-content:space-between;margin-bottom:28px;flex-wrap:wrap;gap:12px; }
.btn-back { display:inline-flex;align-items:center;gap:6px;padding:8px 16px;background:var(--white);color:var(--gray-600);border:1.5px solid var(--gray-200);border-radius:var(--radius-sm);font-family:var(--ff-body);font-size:.83rem;font-weight:600;text-decoration:none;transition:all .15s; }
...
.pat-av { width:50px;height:50px;border-radius:50%;background:rgba(255,255,255,.12);display:flex;align-items:center;justify-content:center;font-size:1.4rem;color:rgba(255,255,255,.85);flex-shrink:0;border:2px solid rgba(255,255,255,.2); }
...
.pat-badge { margin-left:auto;padding:5px 14px;border-radius:999px;font-size:.75rem;font-weight:700;letter-spacing:.3px; }
.pat-badge.pending { background:var(--amber-bg);color:var(--amber); }
.pat-badge.paid    { background:var(--green-bg);color:var(--green); }

/* PAID BANNERS */
.fully-paid-banner { background:linear-gradient(135deg,#059669 0%,#10b981 100%);border-radius:var(--radius);padding:28px 32px;display:flex;align-items:flex-start;gap:18px;margin-bottom:24px;box-shadow:0 8px 32px rgba(5,150,105,.25);color:#fff;position:relative;overflow:hidden; }
.fully-paid-banner::before { content:'';position:absolute;right:-20px;bottom:-20px;width:140px;height:140px;border-radius:50%;background:rgba(255,255,255,.08); }
.paid-banner-icon { width:56px;height:56px;border-radius:50%;background:rgba(255,255,255,.2);display:flex;align-items:center;justify-content:center;font-size:1.8rem;flex-shrink:0; }
.paid-banner-title { font-family:var(--ff-head);font-size:1.4rem;color:#fff;line-height:1.1;margin-bottom:6px; }
.paid-banner-sub { font-size:.85rem;color:rgba(255,255,255,.8);line-height:1.5; }
.paid-banner-badge { margin-left:auto;flex-shrink:0;background:rgba(255,255,255,.2);padding:6px 16px;border-radius:999px;font-size:.8rem;font-weight:700;color:#fff;white-space:nowrap;border:1.5px solid rgba(255,255,255,.3); }
.btn-back-billing { display:inline-flex;align-items:center;gap:6px;margin-top:14px;padding:9px 20px;background:rgba(255,255,255,.2);color:#fff;border:1.5px solid rgba(255,255,255,.35);border-radius:var(--radius-sm);font-family:var(--ff-body);font-size:.85rem;font-weight:700;text-decoration:none;transition:all .15s; }
.btn-back-billing:hover { background:rgba(255,255,255,.3);color:#fff; }

/* CARD */
.card { background:var(--white);border-radius:var(--radius);box-shadow:var(--shadow);margin-bottom:20px;overflow:hidden;border:1px solid var(--gray-200);animation:slideUp .35s ease both; }
...
.card-header { padding:16px 24px;border-bottom:1px solid var(--gray-100);display:flex;align-items:center;gap:12px;background:var(--gray-50); }
...
.items-table thead th { padding:11px 20px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;color:var(--gray-400);text-align:left;border-bottom:1px solid var(--gray-200);white-space:nowrap; }
...
.tot-row { display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-radius:var(--radius-sm);margin-bottom:5px;font-size:13.5px; }
...
.pay-option.cash button { background:none;border:none;width:100%;display:flex;flex-direction:column;align-items:center;gap:10px;cursor:pointer;padding:0; }

/* HISTORY TABLE */
.hist-table { width:100%;border-collapse:collapse; }
.hist-table thead th { padding:11px 20px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--gray-400);border-bottom:1px solid var(--gray-200);white-space:nowrap; }
...
.sync-notice { background:var(--sky-bg);border:1px solid #bae6fd;border-radius:var(--radius-sm);padding:10px 16px;font-size:.82rem;color:var(--sky);display:flex;align-items:center;gap:8px;margin-bottom:20px; }
        ...
        <table class="hist-table">
            ...
            <tr>
                ...
                <td style="color:var(--gray-600);">
                    <?= $h['paid_at']
                        ? htmlspecialchars(date('M d, Y  H:i', strtotime($h['paid_at'])))
