...
try {
    ...
    /* =====================================================
       FETCH IS_PWD FROM PATIENT_RECEIPT
    ===================================================== */
    $is_pwd = 0;
    $stmt = $conn->prepare("SELECT is_pwd FROM patient_receipt WHERE billing_id=? LIMIT 1");
    if ($stmt) {
        $stmt->bind_param("i", $billing_id);
        $stmt->execute();
        $pwd_result = $stmt->get_result()->fetch_assoc();
        if ($pwd_result) {
            $is_pwd = (int)($pwd_result['is_pwd'] ?? 0);
        }
        $stmt->close();
    }
...
if (!empty(trim($output))) {
    header('Content-Type: application/json');
    echo json_encode(['status'=>'error','message'=>'Unexpected output: ' . substr($output, 0, 200)]);
}
?>