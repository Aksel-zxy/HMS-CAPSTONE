...
$sql = "SELECT appointment_id, patient_id, appointment_date, purpose, status, notes, doctor_id 
        FROM p_appointments 
        WHERE status != 'Managed' AND doctor_id = ?" .
...
// Fetch all duty assignments for "My Duties" table, only show non-completed and only for logged-in doctor
$duties = [];
$duty_res = $conn->query("SELECT duty_id, appointment_id, doctor_id, bed_id, nurse_assistant, `procedure`, equipment, tools, notes, status, created_at FROM duty_assignments WHERE status != 'Completed' AND doctor_id = " . intval($_SESSION['employee_id']) . " ORDER BY created_at DESC");
...
// $doctors = $prescription->getDoctors(); // Remove, not needed for dropdown
$patients = [];
    ...
    <div class="d-flex">
        ...
        <div class="main">
            ...
            <div class="container-fluid">
                <h2
                    style="font-family:Arial, sans-serif; color:#0d6efd; margin-bottom:20px; border-bottom:2px solid #0d6efd; padding-bottom:8px;">
                    📌Appointments</h2>
                <table class="appointments-table table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="doctor_duty.php?appointment_id=<?= $appt['appointment_id']; ?>"
                                    class="assign-btn btn btn-primary btn-sm">Manage</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                        <?php else: ?>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="bed_id" class="form-select form-select-sm" required>
                                    ...
                                    <option value="<?= $row['bed_id'] ?>">
                                        ...
                                        (<?= htmlspecialchars($row['ward']) ?>, Room
                                    ...
                                    <option value="<?= $row['bed_id'] ?>" disabled style="color:#aaa;">
                                        ...
                                        (<?= htmlspecialchars($row['ward']) ?>, Room
                                        <?= htmlspecialchars($row['room_number']) ?>,
                                        <?= htmlspecialchars($row['bed_type']) ?>) - <?= ucfirst($row['status']) ?>
                                    </option>
                                    <?php endif; ?>
                                    <?php endforeach; ?>
                                </select>
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-info btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#<?= $equipModalId ?>">See</button>
                                <!-- Equipment Modal -->
                            ...
                            <td>
                                <button class="btn btn-info btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#<?= $toolsModalId ?>">See</button>
                                <!-- Tools Modal -->
                            ...
                            <td>
                                <a href="doctor_duty.php?complete_duty_id=<?= $duty['duty_id'] ?>"
                                    ...
                                    onclick="return confirm('Mark this duty as completed?')">Mark as Completed</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                        <?php else: ?>
            ...
            <div class="container-fluid">
                ...
                <div class="content mt-4">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <?php
                            // Only show prescriptions for the logged-in doctor
                            $sql = "SELECT 
            p.prescription_id,
