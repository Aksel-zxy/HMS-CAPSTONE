...
$laboratorist = new Laboratorist($conn);
$uploader = new FileUploader($conn);

$search = $_GET['search'] ?? '';
$status = $_GET['status'] ?? '';
$laboratorists = $laboratorist->getLaboratorists($search, $status);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = $_POST;
    $data['password'] = password_hash($data['password'], PASSWORD_DEFAULT);

    if ($laboratorist->addLaboratorist($data)) {
        $uploader->uploadDocuments($data['employee_id'], $_FILES);
        echo "<script>alert('Laboratorist registered successfully!'); window.location='list_of_laboratorist.php';</script>";
    } else {
        echo "<script>alert('Error registering Laboratorist!');</script>";
    }
}
...
<body>
    ...
    <div class="main">
        ...
        <div class="topbars">
            ...
            <div class="search-bar">
                <form method="GET" action="list_of_laboratorist.php">
        ...
        <!-- ----- Add Doctors ----- -->
        <center>
            <button class="hahaha" onclick="openForm()">Add Laboratorist</button>
        </center>

        <!-- ----- Table ----- -->
        <div class="employees">
            <p style="text-align: center;font-size: 35px;font-weight: bold;padding-bottom: 20px;color: #0047ab;">Laboratorist List</p>
            <table id="LaboratoristTable">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Employee ID</th>
                ...
                <tbody>
                    <?php if ($laboratorists && $laboratorists->num_rows > 0): 
                        $i = 1;
                    ...
                    ?>
                        <tr>
                            <td colspan="8" style="text-align:center;">No Laboratorist found</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <!-- ----- Pop-Up Form (Add Laboratorist) ----- -->
        <div id="popupForm" class="popup-form">
            <div class="form-container">
                <bttn class="close-btn" onclick="closeForm()">X</bttn>
                <center>
                    <h3 style="font-weight: bold;">Add Laboratorist</h3> 
                </center>
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <label for="username"><span style="color: red;font-weight: bold;font-size: 15px;">*</span>Username: &nbsp;&nbsp;&nbsp;&nbsp;<span style="color: red;font-weight: bold;">(l + Employee ID)</span></label>
                    ...
                    <select id="license_type" name="license_type" required>
                        <option value="Registered Medical Technologist">Registered Medical Technologist</option>
                    <select>
                    <br />
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            const table = document.getElementById("LaboratoristTable");
        ...
        function updateUsername() {
            ...
            if (/^\d{3}$/.test(employeeId)) {
                usernameField.value = "l" + employeeId;
            } else {
                usernameField.value = "";
            }
        }
