...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$employee) {
        echo "<script>alert('Janitor not found.'); window.history.back();</script>";
        exit;
    }
    ...
    $documentTypes = [
        'resume'             => 'Resume',
        'diploma'            => 'Vocational Diploma or Certificate',
    ...
    if ($updateSuccess) {
        echo "<script>alert('Electrical Technician and documents updated successfully!'); window.location.href = 'view_electrical_technician.php?employee_id=$employeeId';</script>";
    } else {
        echo "<script>alert('Error updating Electrical Technician. Please try again.'); window.history.back();</script>";
    }
}
