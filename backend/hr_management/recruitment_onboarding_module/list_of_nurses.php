...
$search = $_GET['search'] ?? '';
$status = $_GET['status'] ?? '';
$nurses = $nurse->getNurses($search, $status);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = $_POST;
    $data['password'] = password_hash($data['password'], PASSWORD_DEFAULT);

    if ($nurse->employeeExists($data['employee_id'])) {
        echo "<script>alert('Error: Employee ID already exists!'); history.back();</script>";
        exit;
    }

    if ($nurse->addNurse($data)) {
        $uploader->uploadDocuments($data['employee_id'], $_FILES);
        echo "<script>alert('Nurse registered successfully!'); window.location='list_of_nurses.php';</script>";
    } else {
        echo "<script>alert('Error registering Nurse!');</script>";
    }
}
...
<body>
    ...
    <div class="main">
        ...
        <!-- ----- Add Nurses ----- -->
        <center>
            <button class="hahaha" onclick="openForm()">Add Nurse</button>
        </center>

        <!-- ----- Table ----- -->
        <div class="employees">
            <p style="text-align: center;font-size: 35px;font-weight: bold;padding-bottom: 20px;color: black;">Nurses List</p>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <center>
                                    <a href="view_nurse.php?employee_id=<?= htmlspecialchars($row['employee_id']); ?>" class="view-link">View Details</a>
                                </center>
                            </td>
                        </tr>
                    <?php endwhile; 
                    ...
                    ?>
                        <tr>
                            <td colspan="8" style="text-align:center;">No Nurse found</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <!-- ----- Pop-Up Form (Add Nurses) ----- -->
        <div id="popupForm" class="popup-form">
            <div class="form-container">
                <bttn class="close-btn" onclick="closeForm()">X</bttn>
                <center>
                    <h3 style="font-weight: bold;">Add Nurse</h3> 
                </center>
                ...
                <label for="username"><span style="color: red;font-weight: bold;font-size: 15px;">*</span>Username: &nbsp;&nbsp;&nbsp;&nbsp;<span style="color: red;font-weight: bold;">(n + Employee ID)</span></label>
                ...
                <select id="nurseDepartment" name="nurseDepartment" required>
                    ...
                    <option value="Geriatrics & Palliative Care (Elderly & Terminal Care)">Geriatrics & Palliative Care (Elderly & Terminal Care)</option>
    ...
    <script>
        ...
        function updateUsername() {
            ...
            if (/^\n{3}$/.test(employeeId)) {
                usernameField.value = "n" + employeeId;
            } else {
                usernameField.value = "";
            }
        }
