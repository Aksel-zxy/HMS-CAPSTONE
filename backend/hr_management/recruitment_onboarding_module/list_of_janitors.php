...
$janitor = new Janitor($conn);
$uploader = new FileUploader($conn);

$search = $_GET['search'] ?? '';
$status = $_GET['status'] ?? '';
$janitors = $janitor->getJanitors($search, $status);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = $_POST;
    $data['password'] = password_hash($data['password'], PASSWORD_DEFAULT);

    if ($janitor->employeeExists($data['employee_id'])) {
        echo "<script>alert('Error: Employee ID already exists!'); history.back();</script>";
        exit;
    }

    if ($janitor->addJanitor($data)) {
        $uploader->uploadDocuments($data['employee_id'], $_FILES);
        echo "<script>alert('Janitor registered successfully!'); window.location='list_of_janitors.php';</script>";
    } else {
        echo "<script>alert('Error registering Janitor!');</script>";
    }
}
...
<body>
    ...
    <div class="main">
        ...
        <div class="topbars">
            ...
            <div class="search-bar">
                <form method="GET" action="list_of_janitors.php">
        ...
        <!-- ----- Add Janitor ----- -->
        <center>
            <button class="hahaha" onclick="openForm()">Add Janitor</button>
        </center>

        <!-- ----- Table ----- -->
        <div class="employees">
            <p style="text-align: center;font-size: 35px;font-weight: bold;padding-bottom: 20px;color: #0047ab;">Janitor List</p>
            <table id="JanitorsTable">
                ...
                <tbody>
                    <?php if ($janitors && $janitors->num_rows > 0): 
                        $i = 1;
                        while ($row = $janitors->fetch_assoc()): 
                    ?>
                    <tr>
                        <td><?= $i++ ?></td>
                        ...
                        <td>
                            <center>
                                <a href="view_janitor.php?employee_id=<?= htmlspecialchars($row['employee_id'] ?? '') ?>" class="view-link">
                                    View Details
                                </a>
                            </center>
                        </td>
                    ...
                    ?>
                        <tr>
                            <td colspan="8" style="text-align:center;">No Janitor found</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <!-- ----- Pop-Up Form (Add Janitor) ----- -->
        <div id="popupForm" class="popup-form">
            <div class="form-container">
                <bttn class="close-btn" onclick="closeForm()">X</bttn>
                <center>
                    <h3 style="font-weight: bold;">Add Janitor</h3> 
                </center>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            const table = document.getElementById("JanitorsTable");
        ...
        function updateUsername() {
            ...
            if (/^\n{3}$/.test(employeeId)) {
                usernameField.value = "j" + employeeId;
            } else {
                usernameField.value = "";
            }
        }
