...
class ElectricalTechnician {
    ...
    public function getElectricalTechnicians($search = '', $status = '') {
        $sql = "SELECT * FROM hr_employees WHERE profession = 'Electrical Technician'";
        $conditions = [];
        $params = [];
        $types = '';
    ...
    public function addElectricalTechnician($data) {
    ...
    public function getByEmployeeId($employeeId) {
        $stmt = $this->conn->prepare("SELECT * FROM hr_employees WHERE employee_id = ? AND profession = 'Electrical Technician' LIMIT 1");
