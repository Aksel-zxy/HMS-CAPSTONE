...
class Nurse {
    ...
    public function getNurses($search = '', $status = '') {
        $sql = "SELECT * FROM hr_employees WHERE profession = 'Nurse'";
        $conditions = [];
        $params = [];
        $types = '';
    ...
    public function getByEmployeeId($employeeId) {
        $stmt = $this->conn->prepare("SELECT * FROM hr_employees WHERE employee_id = ? AND profession = 'Nurse' LIMIT 1");
