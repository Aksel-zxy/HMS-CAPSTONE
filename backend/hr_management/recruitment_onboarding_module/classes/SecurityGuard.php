...
class SecurityGuard {
    ...
    public function getSecurityGuards($search = '', $status = '') {
        $sql = "SELECT * FROM hr_employees WHERE profession = 'Security Guard'";
        $conditions = [];
        $params = [];
        $types = '';
    ...
    public function addSecurityGuard($data) {
    ...
    public function getByEmployeeId($employeeId) {
        $stmt = $this->conn->prepare("SELECT * FROM hr_employees WHERE employee_id = ? AND profession = 'Security Guard' LIMIT 1");
