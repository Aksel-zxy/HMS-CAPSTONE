...
class Pharmacist {
    ...
    public function getPharmacists($search = '', $status = '') {
        $sql = "SELECT * FROM hr_employees WHERE profession = 'Pharmacist'";
        $conditions = [];
        $params = [];
        $types = '';
    ...
    public function getByEmployeeId($employeeId) {
        $stmt = $this->conn->prepare("SELECT * FROM hr_employees WHERE employee_id = ? AND profession = 'Pharmacist' LIMIT 1");
