...
class Accountant {
    ...
    public function getAccountants($search = '', $status = '') {
        $sql = "SELECT * FROM hr_employees WHERE profession = 'Accountant'";
        $conditions = [];
        $params = [];
        $types = '';
    ...
    public function getByEmployeeId($employeeId) {
        $stmt = $this->conn->prepare("SELECT * FROM hr_employees WHERE employee_id = ? AND profession = 'Accountant' LIMIT 1");
