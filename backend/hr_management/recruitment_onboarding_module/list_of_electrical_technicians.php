...
require_once 'classes/ElectricalTechnician.php';
...
$electricalTechnician = new ElectricalTechnician($conn);
$uploader = new FileUploader($conn);

$search = $_GET['search'] ?? '';
$status = $_GET['status'] ?? '';
$electricalTechnicians = $electricalTechnician->getElectricalTechnicians($search, $status);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = $_POST;
    $data['password'] = password_hash($data['password'], PASSWORD_DEFAULT);

    if ($electricalTechnician->employeeExists($data['employee_id'])) {
        echo "<script>alert('Error: Employee ID already exists!'); history.back();</script>";
        exit;
    }

    if ($electricalTechnician->addElectricalTechnician($data)) {
        $uploader->uploadDocuments($data['employee_id'], $_FILES);
        echo "<script>alert('Electrical Technician registered successfully!'); window.location='list_of_electrical_technicians.php';</script>";
    } else {
        echo "<script>alert('Error registering Electrical Technician!');</script>";
    }
}
...
<body>
    ...
    <div class="main">
        ...
        <div class="topbars">
            ...
            <div class="search-bar">
                <form method="GET" action="list_of_electrical_technicians.php">
        ...
        <!-- ----- Add Electrical Technician ----- -->
        <center>
            <button class="hahaha" onclick="openForm()">Add Electrical Technician</button>
        </center>

        <!-- ----- Table ----- -->
        <div class="employees">
            <p style="text-align: center;font-size: 35px;font-weight: bold;padding-bottom: 20px;color: #0047ab;">Electrical Technician List</p>
            <table id="ElectricalTechniciansTable">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Employee ID</th>
                ...
                <tbody>
                    <?php if ($electricalTechnicians && $electricalTechnicians->num_rows > 0): 
                        $i = 1;
                        while ($row = $electricalTechnicians->fetch_assoc()): 
                    ?>
                    <tr>
                        <td><?= $i++ ?></td>
                        ...
                        <td>
                            <center>
                                <a href="view_electrical_technician.php?employee_id=<?= htmlspecialchars($row['employee_id'] ?? '') ?>" class="view-link">
                                    View Details
                                </a>
                            </center>
                        </td>
                    ...
                    ?>
                        <tr>
                            <td colspan="8" style="text-align:center;">No Electrical Technician found</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <!-- ----- Pop-Up Form (Add Electrical Technician) ----- -->
        <div id="popupForm" class="popup-form">
            <div class="form-container">
                <bttn class="close-btn" onclick="closeForm()">X</bttn>
                <center>
                    <h3 style="font-weight: bold;">Add Electrical Technician</h3> 
                </center>
                ...
                <input type="text" id="degree_type" name="degree_type" placeholder="N/A if not available" required>
                <br />
                ...
                <label><span style="color: red;font-weight: bold;font-size: 15px;">*</span>Vocational Diploma or Certificate:</label>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            const table = document.getElementById("ElectricalTechnicianTable");
        ...
        function updateUsername() {
            ...
            if (/^\n{3}$/.test(employeeId)) {
                usernameField.value = "el" + employeeId;
            } else {
                usernameField.value = "";
            }
        }
