...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['applicant_id'], $_POST['status'])) {
    ...
    $applicantObj->updateStatus($applicantId, $status);
...
$hiredApplicants = $applicantObj->getReadyForRegistration();
                    ...
                    <tbody>
                        <?php if (!empty($hiredApplicants)): ?>
                            <?php foreach ($hiredApplicants as $applicant): ?>
                                <tr>
                                    <td><?= htmlspecialchars(trim($applicant['first_name'] . ' ' . $applicant['middle_name'] . ' ' . $applicant['last_name'] . ' ' . $applicant['suffix_name'])); ?></td>
                                    ...
                                    <td>
                                        <?php
                                            ...
                                            $dateVal = $latest['update_at'] ?? $latest['interview_date'] ?? null;

                                            echo !empty($dateVal) ? htmlspecialchars($dateVal) : 'N/A';
                                        ?>
                                    </td>
                                    <td>
                                        <form method="POST" action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" style="display:inline;">
                                            ...
                                            <button type="submit" class="btn btn-success btn-sm">Registered</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            <div class="row1">
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="list_of_doctors.php" class="list">List Of Doctors...</a>
                    </div>
                </div>
            ...
            <div class="row2">
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-text">There are currently <strong><?php echo $totalPharma; ?></strong> active pharmacist registered.</p>
                        <a href="list_of_pharmacists.php" class="list">List Of Pharmacist...</a>
                    </div>
                </div>
